...
SeatCapacity=[230*2];  % refer to seat capacity in peak and offpeak for comfort
MaxSeatCapacity=[230*2];  % refer to seat capacity in peak and offpeak
% specify RailDiscomfort variables
RailDiscomfort={};
RailDiscomfort.Varnames={'DiscomfortRail'};
RailDiscomfort.DCValue=DCValue;
CSorPS='CS';
OperationalCost=[66202]; % OperationalCost 40009 is for sweden with 278 seats, while French train is 510*1.9 assuming coupled, so 139456. 40009/278*510*1.9
BanAvgift=[6957,6091]; % BanAvgift for every departure in peak and offpeak
%% run the model
Model=SpecifySupply_SPD2(CostScaleParameter,DCValue,SeatCapacity);
...
TargetDemand=table();
TargetDemand.('All_Rail')=960; % >0 ASC, this means with 0 ASC value, we get a lower demand than the target (2500) in this case. 1992
TargetDemand.('All_Bus')=960/0.182*0.126;  % <0 ASC, this means with 0 ASC value, we get a higher demand than the target (55) in this case.
TargetDemand.('All_Car')=960/0.182*0.693;  % >0 ASC

% update discomfort variables in model.
UpdateSupplyInfo={};
UpdateSupplyInfo.DiscomfortRail=DCValue(1)^(TargetDemand.('All_Rail')/(SeatCapacity(1)*9)); % here we have 12 trains
Model=UppdateModel(Model,UpdateSupplyInfo);

% update population size
Model.PopulationSize.All=TargetDemand.('All_Rail')+TargetDemand.('All_Bus')+TargetDemand.('All_Car');
...
CalibrateParameters.All.Car.ASCCar=2;
...
% frequency improvement 9 trains to 12 trains
UpdateSupplyInfo={};
UpdateSupplyInfo.FirstWaitTimeRail=60/(12)/2;
NewTrialModel=UppdateModel(Model,UpdateSupplyInfo);
...
[ObjectiveFunctionBaseline,...
 ...
 UpdatedModelOptimal,...
 ElasticityOptimal]=OptimizationProblem_SPD_2(Model,DecisionVariable,RailDiscomfort,SeatCapacity,MaxSeatCapacity,BanAvgift,CSorPS);
