...
UserDefinedParameters.DCVarName.Business.Peak='DiscomfortCostBusinessPeakRail';
UserDefinedParameters.DCVarName.Business.Offpeak='DiscomfortCostBusinessOffpeakRail';

UserDefinedParameters.SeatCapacity.Peak=278;
UserDefinedParameters.SeatCapacity.Offpeak=278;

UserDefinedParameters.CSorPS='PS';

UserDefinedParameters.OperationalCost.Peak=40009;
UserDefinedParameters.OperationalCost.Offpeak=22089;

UserDefinedParameters.BanAvgift.Peak=6957;
UserDefinedParameters.BanAvgift.Offpeak=6091;

% CostScaleParameter=[0.01, 0.01*0.00172/0.00305];  % CostScaleParameter for different demand segments
% % CostScaleParameter=[0.01, 0.01];  % CostScaleParameter for different demand segments
% DCValue=[1.6, 1.6, 1.6,1.6];  % discomfort for DiscomfortPrivatePeakRail; DiscomfortPrivateOffpeakRail; DiscomfortBusinessPeakRail; DiscomfortBusinessOffpeakRail
...
% RailDiscomfort={};
% RailDiscomfort.Varnames={'DiscomfortCostPrivatePeakRail','DiscomfortCostPrivateOffpeakRail','DiscomfortCostBusinessPeakRail','DiscomfortCostBusinessOffpeakRail'};
% RailDiscomfort.DCValue=DCValue;
% CSorPS='PS';
% OperationalCost=[40009,22089]; % OperationalCost peak and offpeak
% BanAvgift=[6957,6091]; % BanAvgift for every departure in peak and offpeak
%% run the model
Model=SpecifySupply_Stcokholm_Gothenburg(UserDefinedParameters);
...
TargetDemand=table();
TargetDemand.('Private_Peak_Rail')=1992; % >0 ASC, this means with 0 ASC value, we get a lower demand than the target (2500) in this case. 1992
...
UpdateSupplyInfo={};
UpdateSupplyInfo.DiscomfortCostPrivatePeakRail=UserDefinedParameters.DCValue.Private.Peak.^((TargetDemand.('Private_Peak_Rail')+TargetDemand.('Business_Peak_Rail'))/(UserDefinedParameters.SeatCapacity.Peak*14))-1;
...
CalibrateParameters.Business.Offpeak.Rail.ASCBusinessOffpeakRail=0.1439  ;
CalibrateParameters.Business.Offpeak.Bus.ASCBusinessOffpeakBus=-0.5815  ;
CalibrateParameters.Business.Offpeak.Air.ASCBusinessOffpeakAir=3.4175  ;
CalibrateParameters.Business.Offpeak.Car.ASCBusinessOffpeakCar=1.2797  ;
...
[FinalDiscomfortDiffUnitest]=DiscomfortDifference(Model,UserDefinedParameters);
FinalDiscomfortDiffUnitest
[CapacitatedDemand,CapacitatedUtility,FinalModel,ComfortBefore,ComfortAfter]=CalculateCapacitatedDemand(NewTrialModel,UserDefinedParameters);
% [CapacitatedDemand,CapacitatedUtility,FinalModel,ComfortBefore,ComfortAfter]=CalculateCapacitatedDemand(Model,UserDefinedParameters);
ComfortBefore
ComfortAfter
...
[CS_Logsum]=ConsumerSurplus(DecisionVariable,Model,NewTrialModel);
% CS_Logsum
clear NewTrialModel
...
NewTrialModel=UppdateModel(Model,UpdateSupplyInfo);
[UpdatedModel]=UppdateDiscomfortInModel(NewTrialModel,UserDefinedParameters);
...
% UpdateSupplyInfo={};
% UpdateSupplyInfo.CostPrivateOffpeakAir=1000000; % 403
% UpdateSupplyInfo.CostPrivatePeakAir=1000000;  % 378
% UpdateSupplyInfo.CostBusinessPeakAir=1000000; % 403
% UpdateSupplyInfo.CostBusinessOffpeakAir=1000000;  % 378
% Model=UppdateModel(Model,UpdateSupplyInfo);
%% optimization of the obj function
[ObjectiveFunctionBaseline,ObjectiveFunctionNonInteger,OptimizedDemandNonInteger,FinalDecisionVariablesNonInteger,UpdatedModelNonInteger]=OptimizeSupply_ConstraintReformulation(Model,UserDefinedParameters,DecisionVariable);
...
[CS_logsum,CS_RuleOfHalf]=ConsumerSurplus(DecisionVariable,Model,UpdatedModelFinal);
fprintf('\n')
